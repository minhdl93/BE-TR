...
*Figure 2: Prototype-activated feature maps at the F2 layer for (a) steel corrosion, (b) pothole, and (c) asphalt crack. Columns #1–#4 show four learned prototype activations, each emphasizing distinct defect regions while suppressing background.*

![BE-TR architecture](figures/figure2.png)
*Figure 3: BE-TR model architecture. The PVTv2 backbone extracts four multi-scale feature maps (F1–F4). Three cascaded D2T decoder blocks refine N learnable queries through WCA, PCA, and self-attention, producing intermediate predictions S1–S3 whose weighted sum forms the final output.*
